/* Neon Finanças Pessoais — Service Worker
   Estratégia:
   - App shell (index.html, manifest, ícones): cache-first.
   - Bibliotecas CDN (Chart.js, SheetJS via jsdelivr): cache-first para uso offline.
   - Dados (IndexedDB): ficam no navegador e não passam pela rede.
*/
var CACHE = 'neon-financas-v3';
var PRECACHE = [
  './',
  './index.html',
  './manifest.json',
...
function isCdn(url){
  return /(?:jsdelivr\.net|unpkg\.com|cdnjs\.cloudflare\.com)$/.test(url.hostname);
}
